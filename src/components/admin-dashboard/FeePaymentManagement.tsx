...
const SAMPLE_PAYMENTS = [
  { id: 'NOVA001', fullName: 'Rahul Sharma', courseName: 'UG Mathematics', feeAmount: '5000', status: 'paid', paymentDate: '2026-02-18', phone: '+91 98765 43210', email: 'rahul@example.com', razorpayPaymentId: 'rzp_demo_001' },
  { id: 'NOVA002', fullName: 'Priya Mehta', courseName: 'UG Physics', feeAmount: '5000', status: 'pending', paymentDate: '2026-02-15', phone: '+91 98765 43211', email: 'priya@example.com', razorpayPaymentId: '' },
  { id: 'NOVA003', fullName: 'Amit Kumar', courseName: 'GRE Preparation', feeAmount: '8000', status: 'paid', paymentDate: '2026-02-10', phone: '+91 98765 43212', email: 'amit@example.com', razorpayPaymentId: 'rzp_demo_003' },
]
...
export default function FeePaymentManagement() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold" style={{ color: '#196d83' }}> Fee Payment Management</h2>
        <p className="text-gray-600 mt-2">Track and manage student fee payments (auto-synced from frontend)</p>
      </div>
      {/* Stats */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Records', value: payments.length, color: '#196d83', bg: '#f0f7fa' },
          { label: 'Total Amount', value: `₹${getTotalAmount().toLocaleString()}`, color: '#196d83', bg: '#f0f7fa' },
        ...
        <div className="flex flex-wrap gap-2">
          {['all', 'pending', 'paid', 'failed'].map(status => (
            <button key={status} onClick={() => setFilterStatus(status)}
              ...
              style={filterStatus === status ? { backgroundColor: '#196d83' } : {}}>
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden">
        <div className="overflow-x-auto">
          <table className="min-w-full">
            <thead style={{ backgroundColor: '#196d83' }}>
              <tr>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={p.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-4 py-3 font-semibold" style={{ color: '#196d83' }}>{p.fullName || p.studentName || '-'}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <select value={p.status} onChange={e => handleStatusUpdate(p.id, e.target.value)}
                        className="px-2 py-1 border rounded text-xs" style={{ borderColor: '#196d83' }}>
