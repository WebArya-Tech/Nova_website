...
const DUMMY = {
  ...
  grade: 'UG',
  review: 'Excellent teaching methodology! My concepts became crystal clear through iThinkLearn classes. Highly recommended!',
  rating: 5,
  date: '2024-01-15T10:00:00.000Z',
  status: 'approved',
  detailedRatings: Object.fromEntries([
    'teachingQuality','personalAttention','testSystem','overallExperience',
    'conceptClarity','doubtSolving','studyMaterial','improvementInConfidence',
    'structuredPlanning','examOrientedPractice','reinforcementClasses','overallSatisfaction',
    'batchSizeAdvantage','individualMonitoring','teacherExperience','resultImprovement'
  ].map(k => [k, 5]))
}
...
export default function ReviewManagement() {
  ...
  const loadReviews = useCallback(() => {
    setLoading(true)
    try {
      const stored = localStorage.getItem('icfy_reviews')
      if (!stored) {
        localStorage.setItem('icfy_reviews', JSON.stringify([DUMMY]))
        setAllReviews([DUMMY])
      } else {
  ...
  const saveAndRefresh = (updated) => {
    localStorage.setItem('icfy_reviews', JSON.stringify(updated))
  ...
  const updateStatus = (id, newStatus) => {
    ...
    if (viewModal?.id === id) setViewModal(prev => ({ ...prev, status: newStatus }))
  }
  ...
  const filtered = selectedStatus === 'all'
    ? allReviews
    : allReviews.filter(r => (r.status || 'pending').toLowerCase() === selectedStatus)

  const counts = {
    all: allReviews.length,
    pending: allReviews.filter(r => (r.status || 'pending') === 'pending').length,
