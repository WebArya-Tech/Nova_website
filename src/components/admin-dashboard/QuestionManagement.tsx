...
export default function QuestionManagement() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-3xl font-bold" style={{ color: '#196d83' }}>❓ Q&A Management</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 border-l-4" style={{ borderLeftColor: '#196d83' }}>
          ...
          <p className="text-4xl font-bold" style={{ color: '#196d83' }}>
      ...
      <div className="bg-white rounded-xl shadow-md p-4 flex flex-wrap gap-3">
        <button
          ...
          className={`px-4 py-2 rounded-lg font-semibold transition-all ${
            filterStatus === 'all' ? 'text-white shadow-md' : 'text-gray-700 hover:bg-gray-50'
          }`}
          style={{ backgroundColor: filterStatus === 'all' ? '#196d83' : 'transparent', border: '2px solid #196d83' }}
        >
          All Questions
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-lg font-semibold transition-all ${
            filterStatus === 'needs-review' ? 'text-white shadow-md' : 'text-gray-700 hover:bg-gray-50'
          }`}
          style={{ backgroundColor: filterStatus === 'needs-review' ? '#dc3545' : 'transparent', border: '2px solid #dc3545' }}
        >
          Needs Review
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-lg font-semibold transition-all ${
            filterStatus === 'pending' ? 'text-white shadow-md' : 'text-gray-700 hover:bg-gray-50'
          }`}
          style={{ backgroundColor: filterStatus === 'pending' ? '#ffc107' : 'transparent', border: '2px solid #ffc107' }}
        >
          Pending
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-lg font-semibold transition-all ${
            filterStatus === 'ai-answered' ? 'text-white shadow-md' : 'text-gray-700 hover:bg-gray-50'
          }`}
          style={{ backgroundColor: filterStatus === 'ai-answered' ? '#28a745' : 'transparent', border: '2px solid #28a745' }}
        >
          AI Answered
        </button>
      </div>
      ...
      <div className="space-y-4">
        {paginatedQuestions.map((q) => (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2 flex-wrap">
                  <h3 className="text-xl font-bold" style={{ color: '#196d83' }}>{q.subject}</h3>
                  <span
            ...
            {q.aiAnswer && (
              <div className="mb-4 p-4 rounded-lg" style={{ backgroundColor: '#fef9f0' }}>
                <p className="font-semibold mb-2" style={{ color: '#196d83' }}>🤖 AI Generated Answer:</p>
