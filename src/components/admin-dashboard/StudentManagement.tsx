...
const loadStudents = () => {
  try {
    ...
    const adminStudents = JSON.parse(localStorage.getItem('nova_admin_students') || 'null')
    if (adminStudents && adminStudents.length > 0) return adminStudents
    // Map nova_users (registered via signup) to student format
    const registered = JSON.parse(localStorage.getItem('nova_users') || '[]')
...
export default function StudentManagement() {
  ...
  useEffect(() => {
    localStorage.setItem('nova_admin_students', JSON.stringify(students))
  }, [students])
  ...
  const handleAddStudent = (e) => {
    ...
    // Also register in nova_users so student can log in
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold" style={{ color: '#196d83' }}>👨‍🎓 Student Management</h2>
          <p className="text-gray-600 mt-2">Manage all registered students</p>
        </div>
        <button
          onClick={() => setShowAddModal(true)}
          className="px-6 py-3 rounded-lg text-white font-bold shadow-md hover:opacity-90 transition-all w-full md:w-auto"
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-white rounded-xl shadow-md p-6 border-l-4" style={{ borderLeftColor: '#196d83' }}>
              ...
              <p className="text-4xl font-bold" style={{ color: '#196d83' }}>{students.length}</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-md p-6 border-l-4" style={{ borderLeftColor: '#ddaa2c' }}>
              ...
              <p className="text-4xl font-bold" style={{ color: '#ddaa2c' }}>{students.filter(s => s.status === 'pending').length}</p>
            </div>
          </div>
          {/* Search and Filter */}
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b-2" style={{ borderColor: '#196d83', backgroundColor: '#fef9f0' }}>
                    <th className="text-left py-4 px-6 font-bold" style={{ color: '#196d83' }}>Student ID</th>
                    <th className="text-left py-4 px-6 font-bold" style={{ color: '#196d83' }}>Name</th>
                    <th className="text-left py-4 px-6 font-bold" style={{ color: '#196d83' }}>Email</th>
                    <th className="text-left py-4 px-6 font-bold" style={{ color: '#196d83' }}>Phone</th>
                    <th className="text-left py-4 px-6 font-bold" style={{ color: '#196d83' }}>Status</th>
                    <th className="text-left py-4 px-6 font-bold" style={{ color: '#196d83' }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
      ...
      {showAddModal && (
        ...
        >
          ...
          >
            <h2 className="text-2xl font-bold mb-6" style={{ color: '#196d83' }}>Add New Student</h2>
