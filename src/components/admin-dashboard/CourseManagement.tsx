...
const loadCourses = () => {
  try {
    const saved = JSON.parse(localStorage.getItem('nova_courses') || 'null')
...
export default function CourseManagement() {
  ...
  useEffect(() => { localStorage.setItem('nova_courses', JSON.stringify(courses)) }, [courses])
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold" style={{ color: '#196d83' }}>📚 Course Management</h2>
      ...
      <div className="bg-white rounded-xl shadow-md p-4 md:p-8">
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="space-y-4 mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <button
              type="submit"
              className="col-span-full px-4 py-2 rounded bg-[#196d83] text-white font-bold"
            >
            ...
            {courses.map((c) => (
              <div key={c.id} className="border rounded-lg p-4 hover:shadow-lg transition">
                ...
                <div className="flex gap-2">
                  <button
                    className="flex-1 px-3 py-1 rounded bg-[#196d83] text-white text-sm"
