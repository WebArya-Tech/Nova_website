...
export default function ForgotPasswordModal({ isOpen, onClose, onOpenLogin }) {
  const { isEmailRegistered, resetPassword } = useAuth()
  ...
  const handleSendOtp = (e) => {
    ...
    if (!isEmailRegistered(email)) { setEmailError('No account found with this email address'); return }
    setIsLoading(true)
    setTimeout(() => {
      sendOtpEmail(email)
      setIsLoading(false)
  ...
  const handleResetPassword = (e) => {
    ...
    setTimeout(() => {
      ...
      else { setPasswordErrors({ newPassword: result.message }) }
    }, 500)
  }
